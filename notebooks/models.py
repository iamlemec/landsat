...
def gen_dual_high(K, C):
    ...
    layer3 = keras.layers.Dropout(0.25)(layer2)
    layer4 = keras.layers.Conv2D(filters=32, kernel_size=8, activation='relu')(layer3)
    layer5 = keras.layers.MaxPooling2D(pool_size=4)(layer4)
    layer6 = keras.layers.Dropout(0.25)(layer5)
    layer7 = keras.layers.Flatten()(layer6)
    layer8 = keras.layers.Dense(units=64, activation='relu')(layer7)
    layer9 = keras.layers.Dropout(0.5)(layer8)
    output_prod = keras.layers.Dense(units=1)(layer9)
...
def gen_depth_high(K, C):
    input_fid = keras.layers.Input(shape=(1,), name='fid')
    input_map = keras.layers.Input(shape=(K, K, C), name='map')
    layer1 = keras.layers.DepthwiseConv2D(depth_multiplier=4, kernel_size=8, activation='relu')(input_map)
    layer2 = keras.layers.MaxPooling2D(pool_size=8)(layer1)
    layer3 = keras.layers.Dropout(0.25)(layer2)
    layer4 = keras.layers.DepthwiseConv2D(depth_multiplier=4, kernel_size=8, activation='relu')(layer3)
    layer5 = keras.layers.MaxPooling2D(pool_size=4)(layer4)
    layer6 = keras.layers.Dropout(0.25)(layer5)
    layer7 = keras.layers.Flatten()(layer6)
    layer8 = keras.layers.Dense(units=64, activation='relu')(layer7)
