...
def load_asie_firms(year, landsat):
    ...
    firms['log_tfp_resid'] = smf.ols('log_tfp ~ C(sic2)', data=firms).fit().resid
...
def load_census_firms(year, landsat):
    # load in firm and location data
    firms = pd.read_csv('../firms/census{year}_geocode.csv', usecols=cols).rename(cols, axis=1)
    targ = pd.read_csv('../index/census{year}_{landsat}.csv', usecols=['id', 'lat_wgs84', 'lon_wgs84', 'prod_id'])
